...
// ManualPrecipitationStations SDK retry feature
...
class ManualPrecipitationStationsRetryFeature extends ManualPrecipitationStationsBaseFeature
{
    ...
    public function init(ManualPrecipitationStationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ManualPrecipitationStationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ManualPrecipitationStationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ManualPrecipitationStationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
