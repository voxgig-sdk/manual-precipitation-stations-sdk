...
//    only - a typed ManualPrecipitationStationsContext there would make every ctx
...
final class ManualPrecipitationStationsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ManualPrecipitationStationsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ManualPrecipitationStationsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ManualPrecipitationStationsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ManualPrecipitationStationsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ManualPrecipitationStationsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ManualPrecipitationStationsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ManualPrecipitationStationsError(is_string($code) ? $code : '', $msg);
    }
