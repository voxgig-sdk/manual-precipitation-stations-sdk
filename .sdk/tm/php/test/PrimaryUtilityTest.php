...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new ManualPrecipitationStationsContext($ctxmap, null);
        ...
        if (isset($ctxmap['spec']) && is_array($ctxmap['spec'])) {
            $ctx->spec = new ManualPrecipitationStationsSpec($ctxmap['spec']);
        }

        // Handle result from JSON map
        if (isset($ctxmap['result']) && is_array($ctxmap['result'])) {
            $ctx->result = new ManualPrecipitationStationsResult($ctxmap['result']);
            if (isset($ctxmap['result']['err']) && is_array($ctxmap['result']['err'])) {
                $msg = $ctxmap['result']['err']['message'] ?? '';
                $ctx->result->err = new ManualPrecipitationStationsError('', $msg);
            }
        }

        // Handle response from JSON map
        if (isset($ctxmap['response']) && is_array($ctxmap['response'])) {
            $ctx->response = new ManualPrecipitationStationsResponse($ctxmap['response']);
    ...
    private static function fixctx(ManualPrecipitationStationsContext $ctx, ManualPrecipitationStationsSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ManualPrecipitationStationsError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ManualPrecipitationStationsError($code, $msg);
    }
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ManualPrecipitationStationsSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new ManualPrecipitationStationsOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ManualPrecipitationStationsSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ManualPrecipitationStationsError) ? $result->err->msg : (string)$result->err;
