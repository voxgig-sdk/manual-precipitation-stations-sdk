...
// ManualPrecipitationStations SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class ManualPrecipitationStationsMakePoint
{
    public static function call(ManualPrecipitationStationsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof ManualPrecipitationStationsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = ManualPrecipitationStationsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ManualPrecipitationStationsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
