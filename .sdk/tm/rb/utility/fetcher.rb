# ManualPrecipitationStations SDK utility: fetcher
...
module ManualPrecipitationStationsUtilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; ManualPrecipitationStationsSDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = ManualPrecipitationStationsUtilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        ManualPrecipitationStationsUtilities.http_discard(http)
        raise
      end
      ManualPrecipitationStationsUtilities.http_checkin(key, http)
