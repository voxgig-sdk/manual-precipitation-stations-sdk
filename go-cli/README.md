# manual-precipitation-stations-cli

boru-driven command-line client **and** interactive REPL for the ManualPrecipitationStations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/manual-precipitation-stations-cli)
make build

# 2. See usage (words, entities, env vars)
./manual-precipitation-stations-cli --help

# 3. Provide credentials once, via the environment
export MANUAL_PRECIPITATION_STATIONS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./manual-precipitation-stations-cli list collection
./manual-precipitation-stations-cli list item

# 5. Override the API base URL for a single call
MANUAL_PRECIPITATION_STATIONS_BASE=https://api.example.com ./manual-precipitation-stations-cli list collection

# 6. No arguments -> interactive REPL
./manual-precipitation-stations-cli
manual-precipitation-stations> list collection
manual-precipitation-stations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/manual-precipitation-stations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/manual-precipitation-stations-cli list collection
   ```
...
Run with no arguments to open a REPL (prompt `manual-precipitation-stations>`). Each line is
evaluated as its own boru expression:

```text
$ ./manual-precipitation-stations-cli
manual-precipitation-stations> list collection
manual-precipitation-stations> /help
manual-precipitation-stations> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `collection`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/manual-precipitation-stations-cli`. |
...
(`./manual-precipitation-stations-cli <expr>`) and interactively (the REPL), and expressions compose the
