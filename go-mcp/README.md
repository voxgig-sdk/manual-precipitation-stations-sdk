# manual-precipitation-stations-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ManualPrecipitationStations SDK as
two agent tools — `manual-precipitation-stations_list` and `manual-precipitation-stations_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/manual-precipitation-stations-mcp)
make build

# 2. Provide credentials via the environment
export MANUAL_PRECIPITATION_STATIONS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user manual-precipitation-stations \
  -- /absolute/path/to/manual-precipitation-stations-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./manual-precipitation-stations-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// manual-precipitation-stations_list: first page of records
{ "entity": "collection" }
{ "entity": "collection", "query": { } }

// manual-precipitation-stations_load: one record by id
{ "entity": "item", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/manual-precipitation-stations-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user manual-precipitation-stations \
     -- "$PWD"/dist/*/manual-precipitation-stations-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `manual-precipitation-stations_list` and `manual-precipitation-stations_load` tools now appear
   in new sessions. Ask the agent to *"list collection using manual-precipitation-stations"*
   and it calls `manual-precipitation-stations_list` with `{"entity":"collection"}`.
...
```sh
export MANUAL_PRECIPITATION_STATIONS_APIKEY=sk_live_xxx            # API key
export MANUAL_PRECIPITATION_STATIONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `manual-precipitation-stations_list` tool
...
### Call the `manual-precipitation-stations_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `manual-precipitation-stations_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `manual-precipitation-stations_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MANUAL_PRECIPITATION_STATIONS_APIKEY` | API key sent with every request. |
| `MANUAL_PRECIPITATION_STATIONS_BASE` | Optional override of the API base URL. |
...
```sh
./manual-precipitation-stations-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"manual-precipitation-stations_load","arguments":{"entity":"item","query":{"id":1}}}}'
```
