...
func main() {
	...
	// Configure from the environment: MANUAL_PRECIPITATION_STATIONS_APIKEY carries the API key and
	// MANUAL_PRECIPITATION_STATIONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MANUAL_PRECIPITATION_STATIONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MANUAL_PRECIPITATION_STATIONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewManualPrecipitationStationsSDK(opts)
