...
func TestItemEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MANUAL_PRECIPITATION_STATIONS_TEST_ITEM_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		itemRef01DataDt0LoadResult := core.ToMapAny(entityData(itemRef01DataDt0Loaded))
...
func itemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MANUAL_PRECIPITATION_STATIONS_TEST_ITEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MANUAL_PRECIPITATION_STATIONS_TEST_ITEM_ENTID"])
	...
	if env["MANUAL_PRECIPITATION_STATIONS_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewManualPrecipitationStationsSDK(core.ToMapAny(mergedOpts))
	}
