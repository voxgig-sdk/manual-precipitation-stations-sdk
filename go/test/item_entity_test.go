...
func TestItemEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MANUALPRECIPITATIONSTATIONS_TEST_ITEM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func itemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MANUALPRECIPITATIONSTATIONS_TEST_ITEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MANUALPRECIPITATIONSTATIONS_TEST_ITEM_ENTID"])
