...
import (
	...
	omni "github.com/voxgig-sdk/manual-precipitation-stations-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ManualPrecipitationStationsError); is {
				out := map[string]any{
					"name":    "ManualPrecipitationStationsError",
...
func omniCtx(arg any, client *sdk.ManualPrecipitationStationsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ManualPrecipitationStationsSDK); is {
			client = live
