...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MANUAL_PRECIPITATION_STATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("MANUAL_PRECIPITATION_STATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MANUAL_PRECIPITATION_STATIONS_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.ManualPrecipitationStationsSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ManualPrecipitationStationsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ManualPrecipitationStationsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ManualPrecipitationStationsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ManualPrecipitationStationsError{Msg: msg, Code: code}
}
