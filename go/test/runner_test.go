...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MANUALPRECIPITATIONSTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("MANUALPRECIPITATIONSTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MANUALPRECIPITATIONSTATIONS_TEST_EXPLAIN"); explain != "" {
