# ManualPrecipitationStations Lua SDK



The Lua SDK for the ManualPrecipitationStations API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Collection()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
The main module (`manual-precipitation-stations_sdk`) exports the SDK constructor
