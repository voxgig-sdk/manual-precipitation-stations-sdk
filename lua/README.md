...
The Lua SDK for the ManualPrecipitationStations API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Collection(nil):list(nil, nil)
...
local result, err = client:ManualPrecipitationStations(nil):load(
  { id = "test01" }, nil
)
