...
// ManualPrecipitationStations SDK Item entity
...
class ItemEntity
{
    ...
    {
        if ($args) {
            $this->_data = ManualPrecipitationStationsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = ManualPrecipitationStationsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single Item.
     *
     * @param ItemLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ItemLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Item|array The loaded Item as an assoc-array at the
     *   SDK boundary; throws ManualPrecipitationStationsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ManualPrecipitationStationsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws ManualPrecipitationStationsError on failure (item-5 convention).
     */
