...
// ManualPrecipitationStations SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ManualPrecipitationStationsTestFeature extends ManualPrecipitationStationsBaseFeature
{
    ...
    public function init(ManualPrecipitationStationsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ManualPrecipitationStationsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ManualPrecipitationStationsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ManualPrecipitationStationsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ManualPrecipitationStationsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ManualPrecipitationStationsParam::call($ctx, $k);
