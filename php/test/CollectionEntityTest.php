...
class CollectionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = ManualPrecipitationStationsSDK::test($seed, null);
        $seen = iterator_to_array($base->Collection(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = ManualPrecipitationStationsConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = ManualPrecipitationStationsSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MANUAL_PRECIPITATION_STATIONS_TEST_COLLECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function collection_basic_setup($extra)
{
    ...
    $client = ManualPrecipitationStationsSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("MANUAL_PRECIPITATION_STATIONS_TEST_COLLECTION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MANUAL_PRECIPITATION_STATIONS_TEST_COLLECTION_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["MANUAL_PRECIPITATION_STATIONS_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new ManualPrecipitationStationsSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["MANUAL_PRECIPITATION_STATIONS_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
