...
// ManualPrecipitationStations SDK feature test
//
...
class FtCtrl extends ManualPrecipitationStationsControl
{
...
class FtClient
{
    ...
    private ManualPrecipitationStationsUtility $utility;

    public function __construct(ManualPrecipitationStationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): ManualPrecipitationStationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ManualPrecipitationStationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ManualPrecipitationStationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ManualPrecipitationStationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ManualPrecipitationStationsConfig::make_feature($name);
    ...
    private static function buildUrl(ManualPrecipitationStationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ManualPrecipitationStationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ManualPrecipitationStationsResponse($fetched);
        }
    ...
    private function fail(ManualPrecipitationStationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ManualPrecipitationStationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
