...
The Python SDK for the ManualPrecipitationStations API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Collection(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.ManualPrecipitationStations(None).load(
    {"id": "test01"}, None
)
