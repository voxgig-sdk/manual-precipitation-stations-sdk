# ManualPrecipitationStations SDK test runner
...
class ManualPrecipitationStationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ManualPrecipitationStationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ManualPrecipitationStationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ManualPrecipitationStationsTestRunner.getenv("MANUAL_PRECIPITATION_STATIONS_TEST_LIVE")
        override = ManualPrecipitationStationsTestRunner.getenv("MANUAL_PRECIPITATION_STATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ManualPrecipitationStationsTestRunner.getenv(key)
        ...
        explain = ManualPrecipitationStationsTestRunner.getenv("MANUAL_PRECIPITATION_STATIONS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ManualPrecipitationStationsTestRunner._test_control is not None:
            return ManualPrecipitationStationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ManualPrecipitationStationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ManualPrecipitationStationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ManualPrecipitationStationsTestRunner.load_test_control()
...
def load_env_local():
    ManualPrecipitationStationsTestRunner.load_env_local()


def env_override(m):
    return ManualPrecipitationStationsTestRunner.env_override(m)


def entity_data(v):
    return ManualPrecipitationStationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ManualPrecipitationStationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ManualPrecipitationStationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ManualPrecipitationStationsTestRunner.load_test_control()


def live_client_options():
    return ManualPrecipitationStationsTestRunner.live_client_options()


def live_delay_ms():
    return ManualPrecipitationStationsTestRunner.live_delay_ms()
