...
require_relative 'ManualPrecipitationStations_types'
...
class ManualPrecipitationStationsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ManualPrecipitationStationsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ManualPrecipitationStationsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ManualPrecipitationStationsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ManualPrecipitationStationsError.new(
        "#{op}_allow",
        "ManualPrecipitationStationsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ManualPrecipitationStationsError => err
    ...
    if fetched.is_a?(Hash)
      status = ManualPrecipitationStationsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ManualPrecipitationStationsError.new(
        "graphql_error", "ManualPrecipitationStationsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ManualPrecipitationStationsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
