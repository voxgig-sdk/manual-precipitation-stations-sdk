# ManualPrecipitationStations Ruby SDK



The Ruby SDK for the ManualPrecipitationStations API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Collection` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ManualPrecipitationStationsSDK.test(testopts, sdkopts)
```
...
raise a `ManualPrecipitationStationsError` (a `StandardError` subclass), so wrap
...
| Field | Type | Description |
| --- | --- | --- |
| `assets` | `Hash` | Assets associated with this item (e.g., CSV data file) |
| `features` | `Array` |  |
| `geometry` | `Hash` | GeoJSON geometry of the station location |
| `id` | `String` |  |
| `links` | `Array` |  |
...
The main module (`ManualPrecipitationStations_sdk`) exports the SDK class
