...
class ItemEntity
  ...
  def data_set(args)
    if args
      @_data = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Item.
  #
  # @param reqmatch [ItemLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Item.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Item, Hash] the loaded Item; raises ManualPrecipitationStationsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ManualPrecipitationStationsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Item>, Array] the matching Item items; raises ManualPrecipitationStationsError on failure
