...
class CollectionEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = ManualPrecipitationStationsSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MANUAL_PRECIPITATION_STATIONS_TEST_COLLECTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def collection_basic_setup(extra)
  ...
  entid_env_raw = ENV["MANUAL_PRECIPITATION_STATIONS_TEST_COLLECTION_ENTID"]
  ...
  if env["MANUAL_PRECIPITATION_STATIONS_TEST_LIVE"] == "TRUE"
    ...
    client = ManualPrecipitationStationsSDK.new(Helpers.to_map(merged_opts))
  end
