...
module ManualPrecipitationStationsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ManualPrecipitationStationsResponse.new({})
    ...
    if fetch_err
      ...
      response = ManualPrecipitationStationsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ManualPrecipitationStationsResponse.new(fetched)
    else
