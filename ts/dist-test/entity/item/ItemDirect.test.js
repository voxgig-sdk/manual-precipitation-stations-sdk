...
(0, node_test_1.describe)('ItemDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.ManualPrecipitationStationsSDK({
    ...
    (0, node_test_1.test)('direct-load-item', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-item', setup.live))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'collections/ch.meteoschweiz.ogd-nime/items',
                method: 'GET',
                params: {},
            });
        ...
        const result = await client.direct({
            path: 'collections/ch.meteoschweiz.ogd-nime/items/{id}',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.MANUAL_PRECIPITATION_STATIONS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ManualPrecipitationStationsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['MANUAL_PRECIPITATION_STATIONS_TEST_ITEM_ENTID'];
    ...
    const client = new __1.ManualPrecipitationStationsSDK({
        base: 'http://localhost:8080',
