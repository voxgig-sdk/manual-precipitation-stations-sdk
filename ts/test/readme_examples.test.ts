...
//     (\`new ManualPrecipitationStationsSDK(...)\` / \`ManualPrecipitationStationsSDK.test(...)\` become
//     \`ManualPrecipitationStationsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ManualPrecipitationStationsSDK'
...
const TEST_SEED = {"entity":{"collection":{"test01":{"id":"test01"}},"item":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ManualPrecipitationStationsSDK(...)\` and any
// \`ManualPrecipitationStationsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ManualPrecipitationStationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ManualPrecipitationStationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
